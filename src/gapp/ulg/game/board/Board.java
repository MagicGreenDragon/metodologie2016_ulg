...
public interface Board<P>
{
    ...
    default Set<Pos> get(P pm)
    {
    	...
    	return Collections.unmodifiableSet(new_s);
    }
    ...
    default void put(P pm, Pos p, Dir d, int n)
    {
    	if( isModifiable() )
        {
    		...
    		Set<Pos> set_pos = new HashSet<>();
    		Pos ad_p, new_p = p;
    		
    		// Cerca le posizioni adiacenti, controllando se sono nella board
    		for( int i = 0 ; i<n ; i++ )
    		{
    			ad_p = adjacent(new_p, d);
    		...
    		// Mette i pezzi nelle posizioni trovate
    		for( Pos position : set_pos )
    		{
    			put(pm, position);
    		}
        }
