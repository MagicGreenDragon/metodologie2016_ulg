...
/**
 * GameRuler per giocare a Add Game.
 * 
 * @author Daniele Giudice
 * 
 */
public class AddGame implements GameRuler<PieceModel<Species>>
{
    /** Limite di tempo per una mossa */
	private final long time;
	/** Lista giocatori di Add Game */
    ...
    private final List<PieceModel<Species>> white_add_list = Arrays.asList(
    		this.white_pawn, this.white_disc, this.white_dama, this.white_knight, this.white_bishop, this.white_rook, this.white_queen, this.white_king );
    ...
    private final List<PieceModel<Species>> black_add_list = Arrays.asList(
	...
	/** View immodificabile della Board di Add Game */
    ...
    /** Crea un GameRuler per fare una partita a Add Game.
    ...
    public AddGame(long time, String p1, String p2)
    {
    	...
    	this.board = new BoardOct<>(3, 3);
    	this.board_view = Utils.UnmodifiableBoard(this.board);
    	
    	// La board iniziale è vuota
    ...
    /**
     * Costruttore che crea un nuovo GameRuler di Add Game a partire da uno esistente
     * @param ot GameRuler del gioco Add Game
     * @throw NullPointerException se ot è null
     */
    private AddGame(AddGame ch)
    {
    ...
    public Set<Move<PieceModel<Species>>> validMoves()
    {
    	...
    	for( Pos p : this.board.positions() )
    	{
    		...
    		for( PieceModel<Species> piece : (this.ally_color.equals("bianco") ? this.white_add_list : this.black_add_list) )
    			moves_set.add( new Move<>(new Action<>(p, piece)) );
    	}
    ...
    @Override
    public GameRuler<PieceModel<Species>> copy() { return new AddGame(this); }
