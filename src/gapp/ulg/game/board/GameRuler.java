...
public interface GameRuler<P>
{
    ...
    default Set<Move<P>> validMoves(Pos p)
    {
    	...
    	Set<Move<P>> moves_set = new HashSet<>();
    	
    	if( board.get(p) == null ) // Nella posizione p non c'è alcun pezzo
    	{
    		for( Move<P> move : validMoves() )
    		{
    			Action<P> a = move.actions.get(0);
    			
    			if( a.kind == Action.Kind.ADD && a.pos.get(0).equals(p) )
    				moves_set.add(move);
    		}
    	}
    	else
    	{
    		for( Move<P> move : validMoves() )
    		{
    			...
    			if( ( (a.kind == Action.Kind.MOVE || a.kind == Action.Kind.SWAP) && a.pos.contains(p) ) || ( a.kind == Action.Kind.JUMP && a.pos.get(0).equals(p) ) )
    				moves_set.add(move);
    		}
    	}
